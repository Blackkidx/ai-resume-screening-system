...
class CompanyService {
  ...
  // =============================================================================
  // HR - UPDATE OWN COMPANY
  // =============================================================================

  async updateMyCompany(data) {
    try {
      const response = await fetch(`${this.baseURL}/api/companies/my-company/update`, {
        method: 'PUT',
        headers: this.getAuthHeaders(),
        body: JSON.stringify(data),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.detail || 'Failed to update company');
  ...
  async uploadCompanyLogo(file) {
    try {
      const token = (await import('./authService')).default.getToken();
      const formData = new FormData();
      formData.append('file', file);
      const response = await fetch(`${this.baseURL}/api/companies/my-company/logo`, {
        method: 'POST',
        headers: { ...(token && { Authorization: `Bearer ${token}` }) },
        body: formData,
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.detail || 'Failed to upload logo');
      return { success: true, data: result, logo_url: result.logo_url };
    } catch (error) {
      return { success: false, error: error.message };
    }
  }
  ...
  async deleteCompanyLogo() {
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.detail || 'Failed to delete logo');
