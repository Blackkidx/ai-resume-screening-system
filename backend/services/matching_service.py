...
class MatchingService:
    ...
    # Major similarity mapping (IT/CS related fields grouped as one)
    SIMILAR_MAJORS = {
        "technology and computing": [
            "computer science", "cs", "วิทยาการคอมพิวเตอร์", "วิทยคอม",
            "information technology", "it", "เทคโนโลยีสารสนเทศ", "ไอที",
            "software engineering", "se", "วิศวกรรมซอฟต์แวร์",
            "computer engineering", "ce", "วิศวกรรมคอมพิวเตอร์", "วิศวะคอม",
            "data science", "ds", "วิทยาศาสตร์ข้อมูล",
            "digital communication", "digital media", "cybersecurity", "network"
        ]
    }
    
    # Fields considered similar to IT/CS (Added Thai keywords)
    RELATED_KEYWORDS = [
        "computer", "engineering", "technology", "software", "data", "digital",
        "คอมพิวเตอร์", "คอม", "วิศวกรรม", "เทคโนโลยี", "ซอฟต์แวร์", "ข้อมูล", "ดิจิทัล", "ไอที", "it", "cs", "se", "ce"
    ]
    ...
    def _calculate_skills_score(
        ...
        if isinstance(skills_data, list):
            # If skills were extracted/stored as a flat list
            all_resume_skills = skills_data
        else:
            # If skills were stored as a structured dictionary
        ...
        if not job_skills:
            logger.info("[MatchingService] No required skills in job - returning 70%")
            return 70.0
    ...
    def _calculate_semantic_skills_score(
        ...
        # Consider a "match" if similarity > 0.65 (stricter — avoids false positives across unrelated domains)
        threshold = 0.65
    ...
    def _calculate_major_score(
        ...
        # Get job required majors — support both single string and list
        majors_required = job_requirements.get("majors_required", [])
        if not majors_required:
            single = job_requirements.get("major_required", "").strip()
            majors_required = [single] if single else []
        ...
        # Score resume major against each required major; take the best
        best_score = 0.0
        for job_major in [m.lower().strip() for m in majors_required if m]:
            # Exact match
        ...
        if best_score > 0:
            logger.info(f"[MatchingService] Best major match score: {resume_major} → {best_score}%")
            return best_score

        logger.info(f"[MatchingService] Different major: {resume_major} vs {majors_required}")
        return 30.0
    ...
    def _calculate_projects_score(
        ...
        # FIX #1: normalize job skills so canonical forms (e.g. "nodejs") match across both sides
        job_skills_norm = self._normalize_skills(job_skills)
        ...
        for project in projects:
            if isinstance(project, str):
                ...
                if not isinstance(project_techs, list):
                    project_techs = [str(project_techs)]
                project_name = project.get("name", "").lower() if isinstance(project.get("name"), str) else ""
                project_desc = project.get("description", "").lower() if isinstance(project.get("description"), str) else ""
            else:
                continue
                
            project_techs_norm = self._normalize_skills(project_techs)
            
            # FIX #1: compare normalized project techs vs normalized job skills
            if any(tech in job_skills_norm for tech in project_techs_norm):
                relevant_count += 1
                continue
            
            # Also check project name/description for keywords (use original lowercase)
    ...
    def _calculate_certification_score(
        ...
        """
        📜 คำนวณคะแนน Certification (10% of total) - AI Pure System

        ลำดับการตัดสิน:

        [Path A] มี LLM analysis จาก cert upload pipeline (AI เป็นตัวตัดสิน):
          1. is_valid_cert=False             → 0%  (ไม่ใช่ cert จริง)
          2. skills_covered ตรงกับ job ≥ 3  → 100% × multiplier
          3. skills_covered ตรงกับ job = 2  → 85% × multiplier
          4. skills_covered ตรงกับ job = 1  → 70% × multiplier
          5. relevance_tags ตรงกับ job      → 60% × multiplier
          6. ไม่ตรง domain/skills เลย        → 0%  (ผิดสาย ไม่ควรได้คะแนน)

        [Path B] ไม่มี LLM analysis (image-only cert):
          - มีไฟล์ + job ต้อง cert เฉพาะ    → 50% (HR review)
          - มีไฟล์ + ไม่ต้อง cert             → 100%
          - ไม่มี cert + ต้อง cert            → 0%

        Multiplier: 1.0 (มีไฟล์แนบ verified) | 0.5 (แค่ข้อความใน resume)
        """
        logger.info("[MatchingService] Calculating certification score (AI Pure System)...")
        ...
        # Job context for domain matching
        job_title = job_requirements.get("title", "").lower()
        job_skills = [s.lower() for s in job_requirements.get("skills_required", []) if s]
        job_skills_norm = self._normalize_skills(job_requirements.get("skills_required", []))
        job_context = f"{job_title} {' '.join(job_skills)}"

        # ── Tier 1: Check required/preferred first (name-based) ──
        # Pull cert names from: LLM extracted name OR resume text extraction OR LLM legacy certifications list
        ...
        # Also include names from LLM cert analysis (more reliable)
        cert_llm_analyses = resume_features.get("cert_llm_analyses", [])
        for analysis in cert_llm_analyses:
            if isinstance(analysis, dict) and analysis.get("cert_name"):
                name = analysis["cert_name"].lower()
                if name not in resume_cert_names:
                    resume_cert_names.append(name)

        if required_certs_lower and resume_cert_names:
            has_required = any(
                any(req in cert_name for cert_name in resume_cert_names)
                for req in required_certs_lower
            )
            if has_required:
                score = 100.0 * cert_multiplier
                logger.info(f"[MatchingService] Required cert matched = {score}%")
                return score

        if preferred_certs_lower and resume_cert_names:
            has_preferred = any(
                any(pref in cert_name for cert_name in resume_cert_names)
                for pref in preferred_certs_lower
            )
            if has_preferred:
                score = 70.0 * cert_multiplier
                logger.info(f"[MatchingService] Preferred cert matched = {score}%")
                return score

        # ── Path A: LLM analysis is available ──
        if cert_llm_analyses:
            logger.info(f"[MatchingService] Using LLM cert analysis ({len(cert_llm_analyses)} cert(s))")

            # Collect all skills_covered and relevance_tags across all uploaded certs
            all_skills_covered: List[str] = []
            all_relevance_tags: List[str] = []
            any_invalid = False
            ...
            for analysis in cert_llm_analyses:
                ...
                is_valid = analysis.get("is_valid_cert", False)
                if not is_valid:
                    any_invalid = True
                    logger.info(f"[MatchingService] is_valid_cert=False for cert '{analysis.get('cert_name')}'")
                    continue  # skip invalid certs entirely

                all_skills_covered.extend(analysis.get("skills_covered", []))
                all_relevance_tags.extend(analysis.get("relevance_tags", []))

            # If ALL certs are invalid docs → 0%
            if not all_skills_covered and not all_relevance_tags:
                if any_invalid:
                    logger.info("[MatchingService] All certs invalid (is_valid_cert=False) = 0%")
                    return 0.0
                # No valid analyses produced any skills/tags
                logger.info("[MatchingService] LLM analyses empty → fallback to 0% (no domain match)")
                return 0.0

            # Check skills_covered vs job skills_required
            skills_covered_norm = self._normalize_skills(all_skills_covered)
            matched_skills = sum(1 for s in skills_covered_norm if s in job_skills_norm)

            if matched_skills >= 3:
                score = 100.0 * cert_multiplier
                logger.info(f"[MatchingService] LLM: {matched_skills} skills matched = {score}%")
                return score

            if matched_skills == 2:
                score = 85.0 * cert_multiplier
                logger.info(f"[MatchingService] LLM: {matched_skills} skills matched = {score}%")
                return score

            if matched_skills == 1:
                score = 70.0 * cert_multiplier
                logger.info(f"[MatchingService] LLM: 1 skill matched = {score}%")
                return score

            # Check relevance_tags vs job title/context
            tags_lower = [t.lower() for t in all_relevance_tags]
            tag_matched = any(tag in job_context for tag in tags_lower)

            if tag_matched:
                score = 60.0 * cert_multiplier
                logger.info(f"[MatchingService] LLM: relevance_tag matched job_context = {score}%")
                return score

            # Cert exists but completely off-domain → 0%
            logger.info(
                f"[MatchingService] LLM: cert has no domain/skills match with job "
                f"(tags={tags_lower}, job_context='{job_context[:60]}') = 0%"
            )
            return 0.0

        # ── Path B: No LLM analysis (image-only cert or LLM unavailable) ──
        logger.info("[MatchingService] No LLM cert analysis — using file-presence fallback")

        if not resume_cert_names:
            # No cert names extracted at all
            if has_cert_files:
                if required_certs_lower or preferred_certs_lower:
                    logger.info("[MatchingService] Image cert + specific req = 50% (HR review)")
                    return 50.0
                logger.info("[MatchingService] Image cert + no req = 100%")
                return 100.0
            if required_certs_lower:
                logger.info("[MatchingService] No cert + required = 0%")
                return 0.0
            logger.info("[MatchingService] No cert + no req = 0%")
            return 0.0

        # Has cert names from resume text but no LLM analysis
        # → use CERT_DOMAIN_MAP as fallback (keyword based)
        verified_tag = "verified" if has_cert_files else "unverified"

        if not required_certs_lower and not preferred_certs_lower:
            score = 100.0 if has_cert_files else 75.0
            logger.info(f"[MatchingService] Has cert names ({verified_tag}), no req = {score}%")
            return score

        # CERT_DOMAIN_MAP fallback — domain bonus
        domain_matched = False
        for cert_name in resume_cert_names:
            for cert_keyword, domains in CERT_DOMAIN_MAP.items():
                if cert_keyword in cert_name:
                    if any(domain in job_context for domain in domains):
                        domain_matched = True
                        logger.info(
                            f"[MatchingService] CERT_DOMAIN_MAP fallback hit: "
                            f"cert='{cert_name}' → domains={domains}"
                        )
                        break
            if domain_matched:
                break
        ...
        if domain_matched:
            score = 60.0 * cert_multiplier
            logger.info(f"[MatchingService] CERT_DOMAIN_MAP fallback domain match ({verified_tag}) = {score}%")
            return score

        # Has cert text but completely irrelevant → 0%
        logger.info(f"[MatchingService] Cert names exist but no domain match ({verified_tag}) = 0%")
        return 0.0
    ...
    def calculate_ai_match(
        ...
        try:
            ...
            logger.info(f"[MatchingService] XGBoost predict: model_available={xgb_result.get('model_available')}")
        except Exception as e:
            logger.warning(f"[MatchingService] XGBoost unavailable: {e}")
            xgb_result = {"model_available": False}

        if xgb_result.get("model_available"):
            xgb_score = xgb_result["xgboost_score"]
            xgb_zone = self._calculate_zone(xgb_score)
            xgb_recommendation = self._generate_recommendation(
                xgb_score, xgb_zone, rule_result["breakdown"]
            )
            logger.info(
                f"[MatchingService] XGBoost result: score={xgb_score:.1f}% zone={xgb_zone.upper()} "
                f"decision={xgb_result['xgboost_decision']} "
                f"confidence={xgb_result['xgboost_confidence']:.2f} "
                f"(rule-based was {rule_result['overall_score']:.1f}%)"
            )
            return {
                **rule_result,
                "ai_method": "xgboost",
        ...
        logger.info("[MatchingService] XGBoost not available — using rule-based result")
        return {
            **rule_result,
