...
/* ─── Tag Input Component ─── */
const TagInput = ({ label, hint, placeholder, tags, onAdd, onRemove, tagColor = 'sky' }) => {
  ...
  return (
    <div>
      ...
      <div className="flex gap-2">
        <input className={inputCls} type="text" value={val} onChange={e => setVal(e.target.value)} placeholder={placeholder} onKeyPress={e => { if (e.key === 'Enter') { e.preventDefault(); if (val.trim()) { onAdd(val.trim()); setVal(''); } } }} />
...
const JobCreation = () => {
  ...
  useEffect(() => {
    ...
    if (!user || !['HR', 'Admin'].includes(user.user_type)) { notify.error('ไม่มีสิทธิ์เข้าถึง'); navigate('/'); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const jobData = {
        ...
        student_levels: formData.year_level.length > 0 ? formData.year_level.map(y => `ปี ${y}`) : ['ปี 3', 'ปี 4']
      };
      delete jobData.year_level;

      const jobUrl = isAdminView
        ? `http://172.18.148.97:8000/api/jobs?company_id=${adminCompanyId}`
        : 'http://172.18.148.97:8000/api/jobs';
      const res = await fetch(jobUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', ...getAuthHeaders() }, body: JSON.stringify(jobData) });
      const result = await res.json();

      if (res.ok) {
        setSuccess('สร้างตำแหน่งงานเรียบร้อยแล้ว!');
        setFormData(emptyForm);
        setTimeout(() => navigate(isAdminView ? `/hr/jobs?company_id=${adminCompanyId}` : '/hr/jobs'), 2000);
      } else {
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="flex items-start gap-4 mb-6 bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <button className="p-2 -ml-1 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors shrink-0 mt-0.5" onClick={() => navigate(isAdminView ? `/hr/dashboard?company_id=${adminCompanyId}` : '/hr/dashboard')} disabled={loading}>
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">สร้างตำแหน่งฝึกงานใหม่</h1>
            <p className="text-sm text-slate-500 mt-0.5">กรอกข้อมูลตำแหน่งงานที่ต้องการเปิดรับสมัคร</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className={sectionCls}>
            ...
            <Field label="ชื่อตำแหน่งงาน" required hint={`${formData.title.length}/5 ตัวอักษรขั้นต่ำ${formData.title.length > 0 && formData.title.length < 5 ? ` (ต้องการอีก ${5 - formData.title.length} ตัว)` : ''}`}>
              <input className={inputCls} type="text" name="title" value={formData.title} onChange={handleInputChange} placeholder="เช่น นักพัฒนาเว็บไซต์, นักวิเคราะห์ข้อมูล" required />
            </Field>
          ...
          <div className={sectionCls}>
            ...
            <TagInput label="ทักษะที่ต้องการ *" placeholder="เช่น Python, JavaScript, React" tags={formData.skills_required} onAdd={v => setFormData(p => ({ ...p, skills_required: [...p.skills_required, v] }))} onRemove={i => setFormData(p => ({ ...p, skills_required: p.skills_required.filter((_, x) => x !== i) }))} tagColor="sky" />
          ...
          <div className="flex gap-3 justify-end">
            <button type="button" className="rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 px-5 py-2.5 text-sm font-semibold transition-colors disabled:opacity-50" disabled={loading} onClick={() => navigate('/hr/dashboard')}>ยกเลิก</button>
            <button type="submit" className="inline-flex items-center gap-2 rounded-lg bg-sky-600 hover:bg-sky-500 text-white px-5 py-2.5 text-sm font-bold transition-colors disabled:opacity-50" disabled={loading}>
              {loading ? <><LoadingSpinner size="small" /> กำลังสร้าง...</> : 'สร้างตำแหน่งงาน'}
            </button>
          </div>
        </form>
      </div>
